...
class RectangularRoom(object):
    ...
    def cleanTileAtPosition(self, pos):
        ...
        pos: a Position
        """
        spot = (int(pos.getX()), int(pos.getY()))
        if spot not in self.cleaned:
            self.cleaned.append(spot)
    ...
    def getRandomPosition(self):
        ...
        """
        arbX = random.uniform(0, self.width)
        arbY = random.uniform(0, self.height)
        return Position(arbX, arbY)
...
##        position = Position.getNewPosition(self.direction, self.speed)
##        spot = (int(position.getX()), int(position.getY()))
##        if spot not in self.cleaned:
##            self.cleaned.append(spot)
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        been cleaned.
        """
        new = self.getRobotPosition().getNewPosition(self.getRobotDirection(), self.speed)
        if self.room.isPositionInRoom(new) == False:
