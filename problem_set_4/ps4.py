...
def r_squared(y, estimated):
    ...
    mean = sum(y)/len(y)
    top, bottom = 0, 0
    yarr, earr = np.array(y), np.array(estimated)
    for n in y:
        bottom += ((n - mean)**2)
...
def evaluate_models_on_training(x, y, models):
    ...
    pylab.xlabel('Years')
    pylab.ylabel('Temps')
    pylab.plot(xValz, yValz, ':b',  )
    for n in models:
        estY = np.polyval(n, x)
        pylab.plot(xValz, estY, '-r')
    R2 = r_squared(y, estY)
    pylab.title('ALBUQUERQUE --> R- square = ' + str(R2))
...
for year in INTERVAL_1:
    y.append(raw_data.get_daily_temp('ALBUQUERQUE', 1, 10, year))
